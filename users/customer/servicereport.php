...
// Check if servicedetails_id is set in the URL
if (isset($_GET['servicedetails_id'])) {
    $servicedetails_id = $_GET['servicedetails_id'];

    // Fetch service details using servicedetails_id
    $service_details = get_service_details_by_servicedetails_id($servicedetails_id);

    if ($service_details) {
        // Fetch shop details using shop_id from the service details
        $shop_details = get_shop_details($service_details['shop_id']);
    }
}

function get_service_details_by_servicedetails_id($servicedetails_id) {
    $conn = dbconnect();
    $sql = "SELECT sd.*, s.service_id, sh.shop_id, 
                   CONCAT(u.first_name, ' ', u.last_name) AS arranger_name, u.profile_img 
            FROM servicedetails sd 
            JOIN services s ON sd.service_id = s.service_id 
            JOIN shops sh ON s.arranger_id = sh.owner_id
            JOIN users u ON s.arranger_id = u.user_id
            WHERE sd.servicedetails_id = ?";
    try {
        $stmt = $conn->prepare($sql);
        $stmt->execute([$servicedetails_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        $shop_id = $service_details['shop_id'];
...
<main class="main">
	<div class="container">
        ...
        <div class="custom-checkbox" style="margin-top:-30px">
            <img alt="<?php echo $service_details['arranger_name']; ?>" src="<?php echo $service_details['profile_img']; ?>">
        </div>
