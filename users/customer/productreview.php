...
// Function to add or update review and feedback in the database
function add_review_feedback($sales_id, $customer_id, $shop_id, $feedback, $rating, $image_names) {
    $table = 'sales';
    $fields = ['customer_id', 'sales_id', 'shop_id', 'feedback', 'rating', 'review_image'];
    $data = [$customer_id, $sales_id, $shop_id, $feedback, $rating, implode(',', $image_names)];

    // Check if a record for the product already exists
    $existing_record = get_existing_record($sales_id, $customer_id);

    if ($existing_record) {
        // Update the existing record
        $where = ['sales_id', 'customer_id'];
        $where_data = [$sales_id, $customer_id];
...
function get_existing_record($sales_id, $customer_id) {
    $conn = dbconnect();
    $sql = "SELECT * FROM sales WHERE sales_id = ? AND customer_id = ?";
    try {
        $stmt = $conn->prepare($sql);
        $stmt->execute([$sales_id, $customer_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        $conn = null;
        return $result;
    } catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $shop_id =  get_shop_id_by_sales_id($sales_id);
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if (!empty($_FILES['images']['name'][0])) {
            $image_names = handle_image_upload($sales_id);
        }

        // Add the review and feedback to the database
        $result = add_review_feedback($sales_id, $customer_id, $shop_id, $feedback, $rating, $image_names);
        if ($result) {
...
function handle_image_upload($sales_id) {
    $image_names = [];
					...
					<?php
						...
						<div class="rating-input">
							<form method="post" class="review-form" id="reviewForm" enctype="multipart/form-data">
								...
								<input type="hidden" name="sales_id" value="<?php echo isset($_GET['sales_id']) ? $_GET['sales_id'] : ''; ?>">
