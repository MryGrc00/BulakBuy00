...
// Add the sales record to the database
function add_sales_record($productID, $shopID, $customer_id, $quantity, $productPrice, $sales_date, $status) {
    ...
    $sql = "INSERT INTO sales (product_id, shop_id, customer_id, amount, sales_date, status) VALUES (?, ?, ?, ?, ?, ?)";
    
    try {
        $stmt = $conn->prepare($sql);
        $stmt->execute([$productID, $shopID, $customer_id, $amount, $sales_date, $status]);
        $conn = null;
        return true;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($selectedProducts as $selectedProduct) {
        ...
        $status = "Pending";
         ...
         echo "Product ID: $productID, Shop ID: $shopID, Quantity: $quantity, Product Price: $productPrice, Calculated Amount: $amount\n";
        // Add the sales record to the database
        $success = add_sales_record($productID, $shopID, $customer_id, $quantity, $productPrice, $amount, $status);
...
<script>
    document.getElementById("placeOrderBtn").addEventListener("click", function() {
        ...
        var isGcashSelected = document.getElementById("gcash").checked;
